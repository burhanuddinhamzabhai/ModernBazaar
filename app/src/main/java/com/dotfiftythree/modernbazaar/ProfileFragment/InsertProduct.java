...
import com.dotfiftythree.modernbazaar.Constants.Verification;
...
public class InsertProduct extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    DatabaseReference verification = firebaseDatabase.getReference("VerificationDB");
                        ...
                        public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                            ...
                            task.addOnSuccessListener(new OnSuccessListener<Uri>() {
                                ...
                                public void onSuccess(Uri uri) {
                                    ...
                                    map = new HashMap<>();
                                    productString = firebaseAuth.getCurrentUser().getUid() + productName.getText().toString() + dateToStr;
                                    ...
                                    map.put(Product.getProductid(), productString);
                                    map.put(Product.getUserid(), FirebaseAuth.getInstance().getCurrentUser().getUid());
                                    product.child(productString).updateChildren(map);
                                    map.clear();
                                    if (productverification.equals("Request")) {
                                        vmap = new HashMap<>();
                                        String vkey = verification.push().getKey();
                                        vmap.put(Verification.getUserID(), firebaseAuth.getCurrentUser().getUid());
                                        vmap.put(Verification.getVerificationID(), vkey);
                                        vmap.put(Verification.getRequestDate(), dateToStr);
                                        vmap.put(Verification.getVerificationProductID(), productString);
                                        vmap.put(Verification.getUserPhone(), userPhone);
                                        verification.child(vkey).updateChildren(vmap);
                                        vmap.clear();
                                    }
